                        ...
                        <tbody className="divide-y divide-slate-100">
                            {expenses.map(item => (
                                <tr key={item.id} className="hover:bg-slate-50 transition-colors group">
                                    <td className="p-4 text-slate-600 text-sm">{new Date(item.date).toLocaleDateString()}</td>
                                    <td className="p-4 text-slate-800 font-medium">
                                        <span className="inline-block px-2 py-1 rounded bg-slate-100 text-slate-600 text-xs font-bold uppercase">{item.category}</span>
                                    </td>
                                    <td className="p-4 text-slate-500 text-sm italic">{item.description || '-'}</td>
                                    <td className="p-4 text-slate-900 font-bold">KSh {parseFloat(item.amount).toLocaleString()}</td>
                            ...
                            {expenses.length === 0 && (
                                <tr>
                                    <td colSpan="5" className="p-12 text-center text-slate-400">No expenses recorded yet.</td>
                                </tr>
                            )}
                        </tbody>
                    </table>
